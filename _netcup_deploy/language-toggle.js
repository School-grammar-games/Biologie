(function () {
  ...
  // Startseite: leere URL (/) oder index.html behandeln
  ...
  var decodedPath = decodeURIComponent(window.location.pathname).replace(/\\/g, '/');
  var explicitPathTarget = null;
  if (/\/Verdauung\/Verdauung\.html$/i.test(decodedPath)) {
    explicitPathTarget = 'Verdauung_EN.html';
  } else if (/\/Verdauung\/Verdauung_EN\.html$/i.test(decodedPath)) {
    explicitPathTarget = 'Verdauung.html';
  }

  var targetUrl = explicitPathTarget
    ? new URL(explicitPathTarget, window.location.href)
    : new URL(encodeURIComponent(targetFile).replace(/%2F/g, '/'), window.location.href);
  targetUrl.search = window.location.search;
  targetUrl.hash = window.location.hash;

  var style = document.createElement('style');
  style.textContent = [
    '.edu-language-toggle{position:fixed;top:12px;right:12px;z-index:2147483000;display:flex;gap:4px;align-items:center;padding:4px;border-radius:999px;background:rgba(255,255,255,.92);box-shadow:0 10px 24px rgba(15,23,42,.18);border:1px solid rgba(15,23,42,.14);backdrop-filter:blur(10px);font:700 12px/1 system-ui,-apple-system,Segoe UI,sans-serif;max-width:220px;overflow:hidden;transition:max-width .18s ease,padding .18s ease,box-shadow .18s ease}',
    ...
    '.edu-language-toggle.is-minimized{width:34px;max-width:34px;height:28px;justify-content:center;gap:0;padding:0;cursor:pointer}',
    '.edu-language-toggle.is-minimized > :not(.active){display:none}',
    '.edu-language-toggle.is-minimized .active{display:inline-flex;align-items:center;justify-content:center;width:18px;height:12px;max-width:18px;padding:0;background:transparent;color:inherit;box-shadow:none;border-radius:2px;pointer-events:auto}',
    '.edu-language-toggle.is-minimized .active span:not(.flag){display:none}',
    '.edu-language-toggle.is-minimized .active .flag{display:block;width:18px;height:12px;margin:0}',
  ...
  function scheduleLanguageToggleMinimize() {
    window.clearTimeout(minimizeTimer);
    minimizeTimer = window.setTimeout(function () {
      toggle.classList.add('is-minimized');
    }, 5000);
  }
  function expandLanguageToggle() {
    toggle.classList.remove('is-minimized');
    scheduleLanguageToggleMinimize();
  }
  toggle.addEventListener('click', function (event) {
    if (toggle.classList.contains('is-minimized')) {
      event.preventDefault();
      event.stopPropagation();
      expandLanguageToggle();
      return;
    }
    scheduleLanguageToggleMinimize();
  }, true);
  toggle.addEventListener('focusin', expandLanguageToggle);
